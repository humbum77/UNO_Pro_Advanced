...
## Защищённый компонент: новая огибающая

Статус: **LOCKED / НЕ ИЗМЕНЯТЬ**.

Новая огибающая v0.9.7-beta и весь связанный с ней код, включая анимацию, защищены полным запретом на изменения без прямого отдельного указания пользователя. Зафиксированы геометрия ADSR, S–R `0.26` (+30%), расчёты координат/времени/уровня, кривые, отрисовка, маркеры, live-индикатор, Note On/Note Off и drag/edit-поведение. Рефакторинг и косвенные изменения этого кода также запрещены.
...
- Этап 1: добавлен чистый расчёт viewport и гибких колонок; адаптируются topbar, Sequencer, Library, Settings, Live Creator и экранная клавиатура.
- SYNTH пока сохраняет фиксированную внутреннюю сетку 1600×1000, потому что геометрия и код новой огибающей имеют статус LOCKED. Их изменение не входит в текущую команду.
- Добавлены unit-проверки layout-математики и исходного кода защищённых функций огибающей.
- Layout unit-тесты на четырёх стандартных разрешениях и lock-тест огибающей: **SOFTWARE PASS**. Полная регрессия: 60/62 PASS; остаются два ранее документированных устаревших ожидания `test_safety_unchanged` и `test_theme`, не связанные с UI-изменением. Визуальная проверка реального Tk-окна: **UNVERIFIED** из-за отсутствия display/Xvfb в среде. Сборка не создавалась.

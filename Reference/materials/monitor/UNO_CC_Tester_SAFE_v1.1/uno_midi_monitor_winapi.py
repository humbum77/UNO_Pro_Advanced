...
class App:
    VERSION = "1.1 CC Tester — Standalone Safe base"
    ...
    def _record_test_send(self, data):
        ...
        rec = (elapsed, "TEST→UNO", "MIDI", len(data), data.hex(' ').upper(),
               f"CC {data[1]} = {data[2]}, ch {(data[0]&15)+1} — output call completed", data)
        with self.record_lock:
            self.records.append(rec)
            n = len(self.records)
        try:
    ...
    def _forward_worker(self):
        while not self.forward_stop.is_set():
            ...
            if self.forward_stop.is_set() or self.stopping:
                continue
            try:
                if direction in ("EDITOR→UNO", "TEST→UNO"):
                    out = self.uno_out
                else:
                    out = self.editor_out
                if out:

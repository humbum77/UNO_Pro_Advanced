...
class CCTests(unittest.TestCase):
    ...
    def test_transport_packets(self):
        a=object.__new__(m.App); a.running=True; a.stopping=False
        a.uno_out=Mock(); a.forward_queue=queue.Queue()
        for data in (b'\xfa',b'\xfb',b'\xfc',bytes.fromhex('F0 7F 7F 06 02 F7'),bytes.fromhex('F0 00 21 1A 02 03 37 00 00 F7')):
            a.send_manual(data)
            self.assertEqual(a.forward_queue.get_nowait(),('TEST→UNO',data))
        self.assertEqual(a.decode(bytes.fromhex('F0 7F 7F 06 01 F7')),'MMC Stop')
        self.assertEqual(a.decode(bytes.fromhex('F0 7F 7F 06 02 F7')),'MMC Play')

    def test_clock_cancel(self):
        a=object.__new__(m.App); a.root=Mock(); a.mark=Mock()
        a.clock_generation=3; a.clock_job='pending'
        a.stop_clock()
        self.assertEqual(a.clock_generation,4)
        self.assertIsNone(a.clock_job)
        a.root.after_cancel.assert_called_once_with('pending')

...
class SessionTests(unittest.TestCase):
    def make(self):
        s=Session();s.song.timeline.insert(0,'preset',3);s.song.timeline.insert(1,'other',2);return s
    ...
    def test_play_start_block(self):
        s=self.make();s.song.timeline.select(5);s.toggle(0);self.assertEqual(s.song.timeline.playhead,4)
    def test_pad_change_preserves_playback(self):
        s=self.make();s.toggle(0);s.select_pad(1);s.update(.5)
        self.assertEqual(s.playing_pad,0);self.assertEqual(s.pads[0].timeline.playhead,2);self.assertEqual(s.song.timeline.length,0)
    ...
    def test_delay_captures_song(self):
        ...
        self.assertEqual(s.playing_pad,0);self.assertEqual(s.pads[0].timeline.playhead,1)

...
def main():
    ...
    files=sorted((ROOT/'live_creator').rglob('*.py'))+[ROOT/name for name in ['run_live_creator.py','run_live_creator.bat','AGENTS.md','UNO_Local_Live_Creator_FINAL_NEXT_BUILD_TASK.md','Docs/LIVE_CREATOR_v0.4-alpha.md','tests/test_live_creator.py','tests/test_v02.py','tests/test_session.py','tests/gui_smoke_v04.py','tests/build_live_creator.py']]
    for path in files:
        if path.suffix=='.py':compile(path.read_text(encoding='utf-8-sig'),str(path),'exec')
    subprocess.run(['git','diff','--check'],cwd=ROOT,check=True)
    release=ROOT/'builds/Live_Creator_v0.4-alpha'
    ...
    (release/'VERSION').write_text('0.4-alpha\n',encoding='utf-8')
    ...
    archive=ROOT/'builds/Live_Creator_v0.4-alpha.zip'

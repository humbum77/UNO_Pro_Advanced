...
Адаптивный UI из `test/v0.9.7-next` принят пользователем и разрешён к переносу в `main`. После fast-forward интеграции тестовая ветка закрывается; `release/v0.9.7-beta` сохраняется как резервная точка без изменений.

...
Новая огибающая v0.9.7-beta и весь код её анимации имеют статус **LOCKED / НЕ ИЗМЕНЯТЬ**. Без прямой отдельной команды пользователя запрещены любые прямые и косвенные изменения, включая геометрию ADSR, S–R `0.26` (+30%), расчёты, отрисовку, Note On/Off, drag/edit, live-индикатор, рефакторинг, оптимизацию и перенос кода.
